...
document.addEventListener('DOMContentLoaded', function () {
        ...
        navigator.mediaDevices.getUserMedia({ audio: true })
            .then(stream => {
                ...
                mediaRecorder.onstop = () => {
                    ...
                    const firstType = audioChunks[0] && audioChunks[0].type ? audioChunks[0].type : (mediaRecorder.mimeType || 'audio/webm');
                    ...
                    (async () => {
                        try {
                            ...
                            const width = graphCanvas ? Math.max(1, Math.floor((graphCanvas.getBoundingClientRect().width))) : 800;
                            fullWaveformPeaks = computePeaksForWidth(decoded, Math.max(1, width - 70));
                    ...
                    (async function uploadAndPredict() {
                        try {
                            ...
                            // Expecting { label: string, score: number }
                            if (resultEl) {
                                resultEl.innerText = `${data.label} (score: ${data.score})`;
                                if (data.label && data.label.toLowerCase().includes('likely')) {
    ...
    function startPlaybackAnalysis() {
        ...
        if (playbackAudioContext) {
            try { playbackAudioContext.close(); } catch (e) { /* ignore */ }
        ...
        try {
            ...
            playbackSource = playbackAudioContext.createMediaElementSource(audioPlayer);
            ...
            audioPlayer.play().then(() => {
                function drawPlaybackWaveform() {
                    ...
                    const width = graphCanvas.clientWidth || graphCanvas.getBoundingClientRect().width;
                    const height = graphCanvas.clientHeight || graphCanvas.getBoundingClientRect().height;
                    ...
                    graphCtx.fillText('Photoacoustic Voice Waveform (Playback)', width / 2, 15);
                    ...
                    if (decodedAudioBuffer && decodedAudioBuffer.length) {
                        const sr = decodedAudioBuffer.sampleRate || fullWaveformSampleRate || 48000;
                        const channelData = decodedAudioBuffer.numberOfChannels > 0 ? decodedAudioBuffer.getChannelData(0) : null;
                        if (channelData) {
                            const winSec = 1.0; // 1 second window
                            const curTime = Math.max(0, Math.min(audioPlayer.currentTime || 0, audioPlayer.duration || 0));
                            ...
                            const e = Math.min(channelData.length, s + Math.floor(winSec * sr));
                    ...
                    if (!audioPlayer.paused && !audioPlayer.ended) {
                        ...
                        try { if (playbackAudioContext) playbackAudioContext.close(); } catch (e) {}
                        playbackAudioContext = null;
                ...
                alert('Please click the audio play button first (user gesture required) and then click "Play & Analyse" again.');
            });
        } catch (err) {
